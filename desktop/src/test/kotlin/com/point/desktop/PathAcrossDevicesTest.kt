 ...
 *
 * Обещанного «оба ядра в одном процессе» модульная стрелка не даёт: телефонная половина живёт
 * в `:executors`/`:data` (Android), десктопная — в `:desktop` (чистый JVM), и подключить одну
 * к другой нельзя даже тестовой зависимостью. Поэтому здесь настоящий компьютер целиком —
 * приёмная, состояние, реестр, исполнитель, разбор письма, — а телефон собран из тех же
 * функций `:core:flow`, которыми его собирает настоящий телефон. Шов, на котором терялся
 * #595, проходит внутри проверяемого.
 *
