...
/**
 * Имя объекта и имя файла — разные вещи (#603).
 *
 * В папке владельца лежало дословно это:
 *
 * ```
 * Счёт 4512 от ООО Ромашка. Оплатить до 20…     ← многоточие в имени, расширения нет
 * объект                                          ← безымянный
 * 20260728_101726 (2).jpg, (3).jpg                ← «(2)» из уникализации
 * ```
 *
 * Телефон называет объекты по содержимому (#533, #581) и присылает такое имя; компьютер брал его
 * для файла. Файл без расширения не открывается двойным щелчком, многоточие ломает часть программ,
 * а «(2)» попадало человеку на экран как часть названия объекта.
 */
