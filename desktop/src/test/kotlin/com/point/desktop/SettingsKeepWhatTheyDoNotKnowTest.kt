...
/**
 * Настройки правят своё и не теряют чужого (#593).
 *
 * Пока настройки жили одним текстовым файлом, запись случалась один раз при первом запуске, и
 * потеря почти не всплывала. Экран настроек делает запись обычным делом — и старое поведение
 * («переписать файл из трёх полей») стёрло бы человеку ключ расшифровки, ключ чтения, свои адреса
 * и модели.
 *
 * Случай не выдуманный: образец `local.properties.sample` показывает, как вписать ключи руками, и
 * имени компьютера в нём нет. Такой файл терялся бы на первом же старте.
 */
class SettingsKeepWhatTheyDoNotKnowTest {
    ...
    @Test fun `стёртый человеком ключ исчезает из файла`() {
        // Обратная сторона бережности: если оставлять всё, что было, стереть ключ станет нечем.
        val home = dir()
