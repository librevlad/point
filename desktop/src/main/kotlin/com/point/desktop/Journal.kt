...
enum class ObjectSource {
    /**
     * Прежняя прямая дорога между устройствами. Нового такого не появляется — дорога с
     * #475 одна, — но значение живо, чтобы прочитать вчерашний журнал: удалить его значило
     * бы потерять память о том, что уже случилось.
     */
    PHONE_LAN,
 ...
 *
 * Дорога у объекта одна (#475), и рассказывать про неё человеку нечего: «с телефона» — весь
 * ответ на вопрос «откуда это здесь взялось». Прежде здесь различались два пути, потому что
 * их было два.
 */
...
/**
 * Сколько станций уже пройдено — по-русски.
 *
 * «2 действия», а не «2 действий»: строка из журнала читается человеком, а не машиной, и
 * машинный счёт в ней сразу выдаёт, что текст никто не писал.
 */
