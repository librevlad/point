...
**Срез «История» — реализован (метрика: минус переключения):**
...
- **Избранных цепочек нет ни одной** (#110 → 05.08.2026). Был `FavoritesStore` с реплеем
  сохранённой последовательности capability; убран целиком вместе с хранилищем, кнопкой
  «★ Сохранить цепочку» и чипами запуска — по решению владельца («избранных цепочек никогда
  не будет»), разбор — в `DECISIONS.md`. Провенанс `FlowFrame(viaCapability/viaTitle)`
  остался: из него строятся лента пути над объектом и снимок флоу (#7).
...
- **Табло прибора — чтения нет ни одного** (#262 → #396). Было отдельное действие «Прочитать
  показание» с собственной геометрией табло и цифровым проходом Tesseract; убрано 04.08.2026
  вместе со всем механизмом по решению владельца («кнопка лишняя»). Замена **пока не написана**:
  внешний глаз на настоящих кадрах приборов даёт ноль из трёх (04.08.2026,
  `docs/VISION-MODELS.md`), и путь от фотографии прибора к показанию заведён задачей **#426**
  (вырезанный барабан с увеличением). Факт `entity.meter` по-прежнему пишет правило формы
  `meterFacts` (`:core:flow/MeterReading.kt`) по распознанному тексту — на квитанциях и в
  переписке, где показание написано словами; знаменатель корпуса правка не трогает (разбор — в
  `DECISIONS.md`).
...
**Срез «Point для ПК» (#147) — реализован; дорога переписана в #475:**
- Новый модуль `:desktop` (Compose Desktop, JVM) — окно, drag&drop, свои действия.
- Протокол `ContinueOnPc` (`:core:flow`, pure): `LinkedPc`, `encodePcFrame`/`decodePcFrame`,
  `encodePcMeta`/`decodePcMeta` — понимание едет метадатой вместе с объектом.
- **Дорога одна — ящики сервера** (#475). Прямого соединения между устройствами нет: ни своего
  HTTP-сервера на ПК, ни mDNS, ни экрана сопряжения, ни ручного адреса. Телефон: пузырь
...
- ПК объявляет свои действия (`RelayRpc.CAPS`, `id=label[<TAB>KIND]` строки);
  телефон кэширует их, когда приезжает круг (`FilePcCaps`), и синтезирует пары
  Capability/Realizer при старте (как app-пузыри #66). Тап → письмо `RelayRpc.OBJECT` с
  именем действия → ПК исполняет realizer и отвечает исходом.
...
**Срез «Progressive Object: корзина» (#96) — был построен, снят 05.08.2026:**
- Как работало: пузырь «В корзину» (терминал) копил объекты в собственном каталоге `basket/`
  (переживал флоу); Home-плашка «Корзина: N» → существующий `ingestMultiple` → обычный
  COLLECTION-флоу («Набор (N)»). Ноль новой механики коллекций.
- Механика унесена в 0.5 целиком вместе с композицией объектов (решение владельца, см.
  `DECISIONS.md`): в коде больше нет ни `Basket`/`FileBasket`, ни `BasketCapability`, ни плашки.
- **`ObjectKind.COLLECTION` остался** — набор рождается из архива и из шаринга нескольких файлов.
  Убран способ собрать набор вручную, а не сама сущность.
...
**Срез «голосовое — объект, а расшифровка — действие» (#223) — реализован:**
- Новый `ObjectKind.AUDIO` (вид, не признак): классификатор узнаёт `audio/*`, `application/ogg`
  и расширения ogg/opus/m4a/mp3/wav/amr/aac/flac/3gp. `entries` — 9 видов, поэтому запись
  file-backed автоматически, и «Поделиться»/«Сохранить»/«Открыть в…»/«На компьютер» приходят
  на неё даром. Дверь Share открыта в манифесте (`audio/*` + `application/ogg`); «Открыть с
  помощью» (`*/*`) и источник «Голос» (#246) работали и раньше.
- Контракт `SpeechToText` (`:core:flow`, чистый) + `Transcription{Heard,Silence}`, строгий
  промпт с маркерами `NO_AUDIO`/`NO_SPEECH`, чистые `parseTranscription`/`transcriptMarkdown`/
  `modelReadableAudio`. Реализация — `LlmSpeechToText` (`:data`) поверх общей цепочки
  `LlmClient`: системного распознавания **из файла** в Android нет (`SpeechRecognizer` слушает
  микрофон) — разбор в `DECISIONS.md`. Офлайновый движок встанет за тот же контракт.
- `TranscribeCapability`/`TranscribeRealizer` — «Расшифровать»: сеть только после тапа
  (`network=true`, `Latency.SLOW` → экран ожидания с отменой), результат один TEXT-объект
  «суть + дословный текст», суть заодно в `semantic.summary` (тот же показ, что у «Понять»).
  Цепочки нет: одно действие человека — один запрос — один объект.
- Долгая запись не молчит: `recordingMinutes`/`listeningStage` (`:core:flow`, чистые) считают
  примерную длительность из веса и типичного битрейта формата и говорят «примерно N мин, это
  займёт время»; у формата с неизвестным битрейтом чисел нет вовсе.
...
- **Движков стало два, и первый — Whisper** (замер 04.08.2026, разбор в `DECISIONS.md`):
  ...
  **тишина останавливает очередь**, а к следующему идут только от исключения; порядок
  ...
  + чистая `parseSummary`); не вышло — суть пустая, и это не ошибка. Имя запроса общее со всеми —
  `pointHeaders`, своего `User-Agent` расшифровка не заводит.

**Срез «внешний глаз» (#280) — реализован (UI не тронут):**
- Новый шов `ExternalEye` (`:core:flow`) + `CloudTextReader` (`:data`): чужой сервис, читающий
  страницу целиком. **Не `LlmClient`** — специальная OCR-ручка промпта не принимает, и в общей
  цепочке моделей «Понять»/«Перевести» на снимке молча получали бы расшифровку вместо ответа.
  **Не `AtomRecognizer`** — тот отдаёт текст с местом на странице, а `mistral-ocr` геометрии текста
  не даёт вовсе.
- В флоу встал за **существующий** шов «несколько Realizer на одну Capability»: «Распознать текст»
  = устройство (10) → внешний глаз (50) → цепочка моделей (90); «Распознать в облаке» = внешний
  глаз (5) → цепочка моделей (10). Пузырьки, подписи и стадии те же.
- Цепочка внешних глаз (`DefaultExternalEye`) — по измеренной эффективности бесплатного:
  `MistralOcrReader` → `OcrSpaceReader` → `OvhVisionReader` (#493; перемер `docs/VISION-MODELS.md`,
  правило «сколько раз ответил → насколько дословно → за сколько»). 402/429 → следующий, никогда в
  кассу. **Двое из трёх без регистрации** (OVH анонимен, OCR.space работает демо-ключом из своих же
  примеров), поэтому цепочка жива и в раздаваемой сборке. Ключ Mistral берётся сначала у человека
  (`keyFor(MISTRAL_PROVIDER_ID)`) и спрашивается на каждом чтении — иначе сильнейший читатель мёртв
  везде, где нет ключа сборки.
- **Сторож вырождения** `degeneratedReading` (`:core:flow`, чистый) — на обоих путях чтения:
  зацикливание, пустой ответ, ответ без букв и цифр → `Failure(recoverable)` «Не смог прочитать».
  Отдельно от `looksLikeOcrGarbage`: та судит кашу движка, эта — выдумку модели.
- **Приватность = настройка человека** (`PrivacyLevel` + `ReaderPrivacy` + `ReaderPromise` + чистые
  `allowedAt`/`allowedBy`, `CloudPrivacySettings` → `PrefsCloudPrivacySettings`): «Максимум
  бесплатного» (умолчание) → «Не учатся на моём» → «Только на телефоне». Средний уровень судит по
  **письменному обещанию** сервиса, а не по флагу страны (#493): «Только Европа» пропускала того,
  кто учится на присланном, и отсекала обещавших не учиться. Отбор сужает цепочку, но не
  переставляет её. Строка на экране `KeyScreen`. Согласие и `cloudDestination` не тронуты.
- `User-Agent` (`pointHeaders`) — во всех исходящих запросах к моделям; его отсутствие стоило нам
  Groq и Cerebras (403 «неизвестно кто»).

**Срез «увеличить мелкий кадр перед чтением» (#273) — реализован:**
- `readingUpscale` (`:core:flow`, чистый) — увеличивать ли кадр и во сколько раз. Порог мелкого
  кадра `READING_FRAME_PX = 2048` — тот же, по которому декодер прореживает большой (одно правило
  с двух сторон); цель по высоте слова `READING_TEXT_PX = 30`; потолок ×4; бюджет памяти 12 Мп.
  Плотность текста — `typicalTextHeightPx` по уже прочитанному слою: знаем буквы — судим по буквам,
  не знаем — по кадру. Сам ресайз за швом `FrameUpscaler<F>` (в тестах фейк умножает числа),
  Android-реализация — в `:data`.
- Шаг встал в трёх местах подготовки картинки: `TesseractTextRecognizer` (офлайновый движок),
  `BitmapOutboundFrames` (кадр облачному читателю — тот же вход намеренно) и `InlineAttachment`
  (вложение зрячей модели — дорога, на которой замер получен).
...
**Срез «атом — свидетель, а не диктор» (#493) — реализован:**
- Текст ячейки перестал быть заложником телефонного движка. `resolveCells` (`:core:flow`) разводит
  три вопроса: **адрес** и **цифры** остаются за атомами (вето страницы, #258 дословно), **буквы и
  оформление** — за тем, кто прочитал лучше. Шум движка вокруг слова (`[6`, `_8.`, `солдат'`) и
  выбор алфавита (`A0998` латиницей) спором не считаются вовсе (`differsOnlyInNoise`,
  `differsOnlyInAlphabet`, `cleanerReading`); настоящий спор букв берёт чтение модели и остаётся
  видимым — ⚠ и оба чтения в дропдауне. Замер на выгрузке владельца: из 21 разошедшейся ячейки
  почерк движка уезжал в файл во всех 21, стало — в 3 (все со спором о цифрах и все помечены).
- Промпт спрашивает метки и чтение как два разных вопроса, и оба всегда: раньше своё чтение
  просили «если заметил ошибку», модель его обычно не давала, и спорить было не с чем.
- **Порог годности** `unfitTable` (`:core:flow`): лист из одного «непрочитанного», непрочитанного
  больше трети слов или спор на трети ячеек — честный отказ «прочитать таблицу не удалось» вместо
  файла-свалки. Треть — то же `WARNING_WALL_SHARE`, которым метрика корпуса объявляет таблицу
  проваленной; у долей есть пол (40 слов, 24 ячейки), чтобы короткий документ не браковался за длину.
- **Шапка** после свода чтений красится по содержимому (`survivedHeaderRows`): строка, поднятая
  выше заголовка выравниванием, больше не выдаётся за шапку.
...
**Срез «один путь между устройствами» (#524, #475) — локальная сеть удалена:**
- **Удалено целиком**: `PcDiscovery`/`AndroidPcDiscovery`/`Advertiser` (mDNS), `PcServer`
  (HTTP-сервер на ПК) и проверка `X-Point-Token`, ручка `/pair` и `PcTransport.pair`,
  `HttpUrlPcTransport`, `HttpPcClipboardSync`, `SelfHealingPcTransport`,
  `LanThenRelayTransport`, `LanThenRelayClipboardSync`, `LinkPath`. Вместе с ними ушли отказы
  про брандмауэр, порты и тип сети.
- **Осталось единственным**: ящики сервера (`POST/GET /mbx/{device_id}`, `POST …/ack?blob=`).
  Клиент — `Mailbox` в `:core:flow` (один на телефон и ПК), поверх него `RelayRpcClient`
  (вопрос-ответ), `RelayPcTransport` и `RelayPcClipboardSync` на телефоне, `RelayPoller` +
  `RelayRequests` на ПК. **Ящик у устройства один**, поэтому вид письма едет в его мете
  (`RelayRpc.KIND`): объект, буфер и вопросы больше не разводятся по выдуманным адресам.
- **Объект едет запросом с ответом**, а не письмом без ответа: только ответ отличает «компьютер
  сделал» от «письмо легло в ящик выключенной машины» (#114, #524).
- **Ключ рождается у устройств** (`DeviceKeys`, ECDH P-256 → SHA-256 → AES-GCM): открытая
  половина едет в круг (`/auth/start`, `/enroll`), закрытая не покидает устройство
  (`EncryptedDeviceKeys` на телефоне, `FileDeviceKeys` на ПК). Общий секрет считают оба конца,
  сервер в нём не участвует — обещание «сервер прочитать не может» держится этим, а не текстом.
  Названная цена: открытые ключи раздаёт сервер, и запоминание ключа при первой встрече плюс
  код устройства для сверки глазами остаются за #474.
- **Связка = общий аккаунт**: круг приезжает с сервера, компьютер из него запоминается
  (`PcLinks`/`FilePcLinks`) — сопряжения как действия нет ни на одном экране.
- **Отказ различает три беды** (`PcUnreachable`): компьютера нет в круге · он не запущен ·
  сервер молчит (плюс размер, названный числом). Слова — `pcUnreachableText` в `:core:flow`,
  одни на пузырёк «На компьютер», действия ПК и общий буфер.
- **Работа без интернета возвращается в 1.1** (#557) — решение владельца 05.08.2026.
