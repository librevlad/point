...
- **Табло прибора — собственного чтения больше нет** (#262 → #396). Было отдельное действие
  «Прочитать показание» с собственной геометрией табло и цифровым проходом Tesseract; убрано
  04.08.2026 вместе со всем механизмом. Прибор читается **обычным распознаванием** — тем же
  «Распознать текст» / «Распознать в облаке», что и всё остальное. Факт `entity.meter` пишет
  правило формы `meterFacts` (`:core:flow/MeterReading.kt`) по распознанному тексту — оно на
  месте, и знаменатель корпуса правка не трогает (разбор — в `DECISIONS.md`).
