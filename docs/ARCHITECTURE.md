...
- Цепочка внешних глаз (`DefaultExternalEye`) — по измеренной эффективности бесплатного:
  `MistralOcrReader` (24/24 дословно, замер `docs/VISION-MODELS.md`) → `OvhVisionReader` (без ключа
  и регистрации). 402/429 → следующий, никогда в кассу.
...
- **Приватность = настройка человека** (`PrivacyLevel` + `ReaderPrivacy` + чистые
  `allowedAt`/`allowedBy`, `CloudPrivacySettings` → `PrefsCloudPrivacySettings`): «Максимум
  бесплатного» (умолчание) → «Только Европа» → «Только на телефоне». Отбор сужает цепочку, но не
