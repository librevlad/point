...
- `findOnPage` (`:core:flow`, чистый) — где на странице лежит запрос человека. Правила сравнения
  **те же, что у свода чтений** (`normConsensus`): регистр/пробелы/кавычки не различие, разрядный
  пробел в числе — оформление, пропавшая запятая — другое число. Никакого нечёткого поиска;
  ищется в пределах строки (`AtomLayer.lines`), находка — набор атомов и рамка, тот же адрес, что
  у выделения (#259) и у ответа модели.
