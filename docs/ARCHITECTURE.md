...
- **Табло прибора** (#262): `MeterReader` (контракт) + `TesseractMeterReader` (`:data`) —
  ...
  чистый `findMeterDisplays` (`:core:flow`, тесты без устройства). Встаёт **средним звеном**
  цепочки «Распознать текст» (10 страница → 50 табло → 90 облако): обычное чтение не
  подменяется, а между «движок не смог» и облаком появляется бесплатный офлайновый шаг.
