...
class PostalAddressTest {
    ...
    fun `строка таблицы, прайса и чека адресом не становится`() {
        // Ревью #262. Форма «слово, слово, небольшое число» — это не только адрес: это ряд
        // прайса, итог чека, шапка таблицы и любая строка CSV с двумя текстовыми колонками.
        // Разбор таблиц — самое частое, чем Point кормят, а `text/csv` для него обычный
        // текстовый объект, поэтому такой «адрес» уезжал и в «Point понял», и в готовность
        // «Построить маршрут». Отличает адрес не число и не длина, а заглавная буква:
        // населённый пункт и улица — имена собственные.
        listOf(
            "Цукор, пачка, 25",
            "Хлб, молоко, 120",
            "Оплата, чек, 4402",
    ...
    fun `у части со строчной буквы допуск держит слово улицы`() {
        // Требование заглавной не имеет права выбросить настоящий адрес: «м. Павлоград» —
        // имя собственное внутри части, «вул. Кодацька» держится словом улицы.
        assertEquals(AddressForm.STREET, addressForm("м. Павлоград, вул. Кодацька, 15"))
        assertEquals(AddressForm.STREET, addressForm("Олексйвка, вул. Сонячна, 15"))
        assertEquals(AddressForm.STREET, addressForm("вул. Сонячна, 15"))
        // И адрес кадра 12, где слова улицы нет вовсе, — тоже на месте.
        assertEquals(AddressForm.STREET, addressForm("Бритвка, Центральна, 586"))
    }
