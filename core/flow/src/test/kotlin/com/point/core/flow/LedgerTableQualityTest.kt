...
/**
 * Кто выигрывает символы ячейки — замер на **настоящей выгрузке владельца** (#493).
 *
 * Слева в каждой паре — то, что реально приехало в `.xlsx` 04.08.2026, дословно из разбора файла:
 * `[6`, `7,`, `_8.`, `А4152_`, `(31.07.2026`, `солдат'`, `Андри Васильович!`. Это почерк телефонного
 * движка, и он попадал в файл потому, что текст ячейки собирался из атомов всегда — независимо от
 * того, как ту же ячейку прочитала зрячая модель.
 *
 * Справа — чтение модели. Оно здесь не выдумано «под ответ»: это те же значения, набранные так, как
 * печатная кириллица читается зрячей моделью на замере (`docs/VISION-MODELS.md`, 24/24 дословно).
 *
 * Замеряется ровно то, что изменилось: **в скольких ячейках в файл уезжает почерк движка**, когда
 * движок и модель прочитали по-разному. Оба состояния воспроизводятся на одном входе: «до» — это
 * ответ без своего чтения (`text = null`), ровно то, что отдавала модель по старому промпту, где
 * чтение просили добавлять, только «если заметил ошибку».
 */
class LedgerTableQualityTest {

    /** Пара «как прочитал телефон» → «как прочитала модель». */
    ...
    /** Слой страницы: одна ячейка — один атом, каждая на своём месте (ячейки не рвутся). */
    ...
    /** Ответ модели: метки всегда, своё чтение — по [withReading]. */
    ...
    /** Ячейки, где движок и модель прочитали по-разному — знаменатель замера. */
    private val differing = ledger.sumOf { row -> row.count { (engine, model) -> engine != model } }

    /** В скольких ячейках в файл уехал почерк движка (среди разошедшихся). */
    ...
    /** Три ячейки, где спор о ЦИФРАХ, — вето страницы (#258) держится дословно. */
    @Test
    ...
    /** Латиница вместо кириллицы — выбор словаря движка, а не спор о значении. */
    @Test
    ...
    /**
     * Пометок должно стать больше, чем было (спор перестал быть невидимым), но не настолько, чтобы
     * лист превратился в задание перепроверить его целиком, — иначе порог годности ([unfitTable])
     * забракует нормально прочитанную страницу.
     */
    @Test
