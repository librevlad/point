...
class CorpusScoreCliTest {
    ...
    /**
     * Три причины — три строки. Одна общая строка со словом «пока» обещала человеку работу по
     * кадрам, схемы которым решили не заводить, и молчала о том, что таблицы уже измерены
     * другим числом.
     */
    @Test
    ...
    /**
     * Сторож против молчаливого сужения корпуса: **ни один** кадр не имеет права стоять вне
     * знаменателя без названной причины. Опечатка в id схемы («rout») тоже ловится здесь — кадр
     * тихо ушёл бы в `unscored` и уменьшил знаменатель, ничем себя не выдав.
     */
    @Test
    ...
    /**
     * Обратная сторона того же сторожа: причина на кадре, у действия которого схема ЕСТЬ, —
     * забытая строка. Число она не меняет (кадр считается), поэтому прогон о ней не скажет
     * никогда, а карта будет врать человеку.
     */
    @Test
    ...
    /**
     * Отказ — решение, а не «пока не дошли руки»: кадры 16 (список покупок в чужом приложении) и
     * 21 (дерево отчётов на чужом мониторе) стоят вне счёта по записанной причине
     * (`docs/DECISIONS.md`, разбор рядом с [ACTION_SCHEMAS]). Тест держит именно решение: тихо
     * поменять отказ на «ждёт схемы» — значит пообещать работу, которой не будет.
     */
    @Test
