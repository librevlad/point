...
class AtomLayerTest {
    ...
    /**
     * Текст всего слоя уедет туда, где сегодня стоит `recognize(): String`, а его потребители
     * режут результат по переводам строки (`layoutOf`). Склеить страницу в одну строку значит
     * отдать им документ без строк — и раскладка развалится ещё до того, как её кто-то увидит.
     */
    @Test
    ...
    /**
     * Двухколоночный документ — место, где пересобранный по полосам текст врёт, а движок прав.
     *
     * Слово левой колонки и слово правой лежат в одной горизонтальной полосе, и сборка по полосе
     * склеит их в одну строку. У Tesseract для этого есть собственный анализ раскладки, и его
     * текст колонки различает. Раз чтение одно и то же — координаты и текст берутся из одного
     * прохода, — доверять надо тому, кто знает больше.
     *
     * Без этого правила переход на атомы стал бы регрессией ровно на тех документах, ради которых
     * всё затевалось: на ведомости в две колонки.
     */
    @Test
