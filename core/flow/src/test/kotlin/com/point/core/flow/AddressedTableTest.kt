...
class AddressedTableTest {
    ...
    // -- гейт диктовки судит цифру, только когда страница способна судить (#294) --

    /**
     * Эталонная ведомость владельца: движок телефона отдал символьную кашу, и страница не
     * подтвердила НИ ОДНОГО из 384 чисел ответа — в файл уехали 385 помеченных ячеек из 448
     * непустых. Пометка, стоящая везде, перестаёт что-либо значить, поэтому страница, которой
     * нечего засвидетельствовать, цифры и не судит.
     */
    @Test
    ...
    /** Обратная сторона: страница, которая числа таблицы действительно читает, диктовку ловит —
     *  иначе правило свидетеля просто отключило бы гейт. */
    @Test
