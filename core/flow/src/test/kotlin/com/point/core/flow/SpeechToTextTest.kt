...
class SpeechToTextTest {
    ...
    // --- Суть для движка, который умеет только слушать ---
    ...
    fun `суть добирается по расшифровке — и остаётся сутью, а не пересказом целиком`() {
        ...
        // Модель, вернувшая ту же расшифровку, ничего не назвала: заголовок «Суть» над ней обещал бы
    ...
    fun `суть не выдумывается, когда её не дали`() {
        ...
        // Короткой записи сокращать нечего — и сути у неё честно не будет.
