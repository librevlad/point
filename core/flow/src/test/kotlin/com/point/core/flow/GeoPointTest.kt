...
/**
 * Координаты — второе чтение требования «куда ехать» (#262). Кадр корпуса «точка по
 * координатам» адреса не имеет вовсе, и без этого факта маршрут по нему считался бы неготовым.
 *
 * Правило намеренно узкое: оно пишет ФАКТ, поэтому точность здесь дороже полноты.
 */
class GeoPointTest {
    ...
    fun `цены и количества координатами не становятся`() {
        // Две колонки накладной стоят рядом ровно так же — их спасает только точность формы.
    ...
    fun `запятая как десятичный разделитель правилу не отдана — гадать оно не будет`() {
        // «50,4501, 30,5234» неразличимо с парой чисел «50,4501» и «30,5234»: правило, которое
        // гадает, врёт дважды. Эта форма — работа модели («Понять», ключ GEO).
