...
/**
 * Ключ вводится один раз и есть на обоих устройствах (#589).
 *
 * Судится правило слияния — единственное место, где два устройства могут разойтись во мнении о
 * том, какой ключ настоящий.
 */
class SharedSecretsTest {
    ...
    @Test fun `непустое побеждает пустое, и неважно, чьё свежее`() {
        ...
        val pc = SharedSecrets(at = 500) // на ПК ключа нет вовсе, но метка свежее
    ...
    @Test fun `ключи разных сервисов сливаются по отдельности`() {
        // Свежий ключ речи не должен затирать давний ключ AI только потому, что приехал позже.
    ...
    @Test fun `пустых ключей в письме нет вовсе`() {
        // Строка `ai.key=` на другом устройстве выглядит как «ключ есть, но сломан».
