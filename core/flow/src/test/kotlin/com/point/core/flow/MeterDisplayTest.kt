...
class MeterDisplayTest {
    ...
    /**
     * Кадры выше сосчитаны для нынешнего [METER_WORK_PX], и связь эту надо держать: все пороги
     * ниже — рост знака, шаг, толщина рамки — подобраны под рабочую копию в 800 точек по длинной
     * стороне. Сменить константу и не пересчитать кадры — значит искать табло на размере,
     * которого телефон никогда не увидит, и не узнать об этом: ревизия #239 убрала прежнюю
     * проверку как «мёртвую», но вместе с копией арифметики `grayFrame` ушла и единственная
     * нить между константой и этими числами (проверено: 800 → 1000 оставляет файл зелёным).
     *
     * Здесь только сама нить: длинная сторона рабочей копии равна константе по определению.
     * Арифметику уменьшения не переписываем — она в `:data`, на Bitmap.
     */
    @Test
    fun `кадры корпуса пересчитаны под нынешний рабочий размер`() {
        assertEquals(METER_WORK_PX, maxOf(waterMeterWork.first, waterMeterWork.second))
        assertEquals(METER_WORK_PX, maxOf(powerMeterWork.first, powerMeterWork.second))
    }
