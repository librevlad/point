...
/**
 * Сомнение в прочитанном (#425) — судится на настоящем документе владельца.
 *
 * Числа взяты с фотографии акта №145 от 04.06.2021 (акт прийому-передачі, снят с рук): девять
 * строк и итог 3684,20. Выдуманная таблица здесь была бы бесполезна — проверка обязана выдержать
 * ровно тот вид, в котором накладные приходят из чтения: markdown-таблицей, с копейками через
 * запятую и пробелом в разрядах.
 */
class ReadingDoubtTest {
    ...
    fun `итог не сошёлся — человеку сказано, на сколько именно`() {
        // Ровно та ошибка, что случилась на замере: строки прочитаны, одна сумма потеряна.
    ...
    fun `пробелы в разрядах не считаются расхождением`() {
        // «3 684,20» и «3684,20» — одно число. Иначе настоящая накладная разойдётся сама с собой.
        assertNull(totalMismatch(akt.replace("3684,20", "3 684,20")))
    }

    @Test
    fun `без строки итога проверка молчит, а не кричит`() {
        // Ложная тревога дороже пропуска: она приучает не читать предупреждения.
    ...
    fun `слово из двух алфавитов названо поимённо`() {
        // «Уайт-Спірit»: хвост латиницей — ровно такую подмену модель делает молча.
    ...
    fun `самое опасное сомнение идёт первым`() {
        // Несошедшийся итог — единственная проверка, поймавшая уверенную ошибку модели на замере.
        // Если он утонет в списке подмен букв, человек до него не дочитает.
        val broken = akt.replace("| 1287,20 |", "| 287,20 |")
