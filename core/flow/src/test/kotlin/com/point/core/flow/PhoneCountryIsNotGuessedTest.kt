...
class PhoneCountryIsNotGuessedTest {
    ...
    /**
     * Тот же номер, прочитанный дважды — с кодом и без, — остаётся одним знанием на экране.
     *
     ...
     *
     * Проверяется здесь то, ради чего сторож и стоял: на экране спора нет. Второе прочтение
     * при этом остаётся при знании (#1303) — потерять его молча нельзя, а строка «или:»
     * показывает расхождение, а не ту же запись другими знаками (#1011).
     */
    @Test
    fun `один номер в двух записях не показан спором ни на одном устройстве`() {
        devices.forEach { device ->
            ...
            val shown = merged.getValue(META_ENTITY_PHONE)

            assertTrue(
                "на устройстве-$device один номер показан спором-${merged[META_ENTITY_PHONE + META_ALT_SUFFIX]}",
                alternativesOf(merged, META_ENTITY_PHONE).none { !sameFact(META_ENTITY_PHONE, it, shown) },
            )
            assertEquals(
                "второе прочтение пропало молча на устройстве-$device",
                fromOklahoma,
    ...
    /**
     * Слились — значит спор: проигравшее прочтение остаётся при знании (#1303).
     *
     ...
     * настоящего немецкого `+49 6763 60560`, хотя между собой эти двое разные. Развести их
     * честно нечем: назвать страну записи без кода и значит угадать (#1029).
     *
     * Раз развести нечем — слияние спорно, и второе прочтение остаётся «или» с происхождением,
     * как всякое другое (решение владельца 31.08.2026). Молчаливой заменой знание не
     * уничтожается: человек больше не теряет номер, которого не видел.
     *
     * Экран при этом не шумит: строка «или:» показывает расхождение, а не ту же запись
     * другими знаками (#1011), — и три записи одного номера остаются на нём одним знанием,
     * как их и закрыл #932.
     */
    @Test
    fun `слившиеся номера остаются оба — проигравшее прочтение не пропадает`() {
        ...
        devices.forEach { device ->
            ...
            listOf(german to fromDocument, fromDocument to german).forEach { (first, second) ->
                ...
                assertEquals(
                    "второе прочтение пропало молча-$device-$graph",
                    second,
