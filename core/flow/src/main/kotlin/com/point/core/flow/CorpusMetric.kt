...
enum class OutOfCount(val word: String, val note: String) {
    ...
    // Отказ — решение, а не «пока». Адрес решения — сама карта кадров: слово «отказ» рядом с
    // кадром и есть запись, другого её изложения в репозитории нет (#1234).
    REFUSED("отказ", "проверять не будем — так решено в карте кадров tools/corpus/frames.tsv"),
