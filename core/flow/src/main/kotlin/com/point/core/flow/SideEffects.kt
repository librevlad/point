...
interface PdfTextExtractor {
    ...
    /**
     * Текст документа — не больше, чем спросили (#1241).
     *
     * [atMost] — сколько знаков нужно спросившему; `null` — нужен весь документ. Длинный PDF
     * разбирался целиком при каждом входе в объект, после каждого действия с находками и на
     * каждую реплику разговора — секунды мёртвого времени ради шестнадцати тысяч знаков,
     * которые всё равно уйдут в вопрос. Норма проекта: не запускать дорогую полную цепочку
     * для каждого объекта.
     *
     * Предел мягкий: читается по странице, пока знаков не хватит, и последняя страница
     * приходит целиком. Резать слово посреди было бы враньём о том, что на странице.
     */
    suspend fun extractText(obj: PointObject, atMost: Int? = null): String
}
...
interface OfficeTextExtractor {
     ...
     *
     * Каждая часть приходит со своим номером — номер слайда принадлежит самому слайду, а не
     * его месту в списке; слайдов, которых в файле нет, нет и здесь. Тот же орган, что и
     * чтение текста, — читатель офисного файла один. Ответ обязателен, а не подразумевается
     * пустотой: молчаливое «слайдов нет» у читателя, который просто не умеет их доставать,
     * человек прочитал бы как «в презентации ничего нет».
     */
    suspend fun slides(obj: PointObject): List<Pair<Int, String>>
}
