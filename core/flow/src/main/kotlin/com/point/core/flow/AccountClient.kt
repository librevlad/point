...
interface AccountClient {
    ...
    /**
     * «Удалить всё моё» — учётная запись, все устройства и все байты сервера, немедленно.
     *
     * Отдельно от [signOut] по существу, а не для полноты: выход снимает **это** устройство и
     * оставляет аккаунт на месте; удаление не оставляет ничего и необратимо. Пока такой двери не
     * было, человек, решивший уйти совсем, мог только выйти — а его почта, круг и невыбранные
     * письма продолжали лежать на сервере.
     *
     * `false` — не дозвонились или сервер отказал: тогда ничего не удалено, и говорить человеку
     * «готово» нельзя.
     */
