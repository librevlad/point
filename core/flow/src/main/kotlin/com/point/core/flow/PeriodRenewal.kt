 ...
 * к нему привязано, стирать нечего и действия нет.
 *
 ...
 *
 * Принимается, только если запись **накрывает** найденный календарь: тогда это действительно
 * период документа, а не пара дат, случайно оказавшихся рядом в примечании.
 */
private fun statedPeriod(rows: List<List<String>>, calendar: DocumentPeriod): DocumentPeriod? {
    rows.forEach { row ->
        row.forEach { cell ->
            ...
            if (found.size == 2) {
                val (from, to) = found
                if (!from.isAfter(calendar.from) && !to.isBefore(calendar.to) && !from.isAfter(to)) {
                    return DocumentPeriod(from, to)
                }
            }
        }
    }
