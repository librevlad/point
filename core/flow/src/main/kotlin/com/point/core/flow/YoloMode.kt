 ...
 * перед соседским и соседское перед чужим. Здесь вперёд идёт тот, кто даёт лучший результат.
 *
 * Лучший результат — у того, кто дотягивается до сильного движка снаружи: чужого сервиса
 * ([RealizerMeta.leavesCircle]), прямо или через свой компьютер. Среди них первым идёт прямой
 * путь в облако, за ним — компьютер, который отправит дальше. Тот же признак, что и у
 * согласия (#1088): уходит наружу тот, кто это объявил, а не всякий, кто не здешний.
 *
 * Компьютер, делающий работу у себя, лучшего результата не даёт (#1415): разбор PDF на
 * страницы, поиск в тексте, новый период у таблицы выходят на нём такими же, как на телефоне,
 * а стоят дороже — объект едет через сервер и ждёт очереди. До #1407 компьютер этих умений не
 * объявлял, и телефон делал их сам; объявил — и в режиме каждое из них уезжало на компьютер
 * только за то, что он сосед. Поэтому среди работающих у себя порядок остаётся бережным:
 * своё, потом сосед. Лестница исполнителей одна на оба режима ([nearness]), и режимы не могут
 * разъехаться по третьему виду исполнителя.
 *
...
fun yoloOrder(candidates: List<Realizer>): List<Realizer> =
    candidates.sortedWith(
        compareBy(
            { if (it.meta.leavesCircle) 0 else 1 },
            { if (it.meta.leavesCircle) -nearness(it.meta.kind) else nearness(it.meta.kind) },
            { it.meta.priority },
            { it::class.java.name },
        ),
    )
