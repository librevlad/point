...
internal fun understandPrompt(
    ...
    append(
        "Если ты не уверен, каких кандидатов на поле несколько — перечисли до " +
            ...
            // Языковое правило сути общее со зрячим и голосовым путём (#1036, история —
            // при самом правиле), но здесь оно только про SUMMARY: значения выше просятся
            // дословно и судятся по словам страницы («нет в тексте — нет знания», #809) —
            // переведённый адрес или имя в тексте не нашлись бы и пропали.
            answerLanguageRule("SUMMARY", "текст") + "\n\n",
    )
