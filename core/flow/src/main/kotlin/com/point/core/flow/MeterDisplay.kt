...
/**
 * Что сказать человеку, пока читается очередное найденное место (#288).
 *
 * «Прочитать показание» — не одно ожидание, а два: сперва перебор наклонов ищет табло (это
 * «Ищу табло прибора»), потом движок отдельным проходом читает КАЖДОЕ найденное место, до трёх.
 * Раньше вторая половина шла молча, хотя она и есть самая долгая, а в логе счёт мест был —
 * строкой `meter: N display candidate(s)`. Число мест известно точно ещё до первого чтения
 * (кандидаты уже отобраны), поэтому «из скольких» здесь — правда, а не обещание.
 *
 * Одно место — числа нет вовсе: «место 1 из 1» сообщает человеку ровно ничего и звучит как отчёт
 * машины о себе.
 */
fun meterPlaceStage(index: Int, total: Int): String =
    if (total <= 1) "Читаю цифры на табло" else "Читаю цифры — место ${index + 1} из $total"
