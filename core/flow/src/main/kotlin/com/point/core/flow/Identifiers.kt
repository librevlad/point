 ...
 *
 * **Какие длины бывают на самом деле** (проверено по кадрам корпуса и по тому, что печатают
 * перевозчики): Нова Пошта — 14 цифр (`59001627808586`, `20 4514 9154 9395`); Укрпошта внутри
 * страны — 13 цифр (`0501234567890`); Укрпошта за границу и всякий международный перевозчик —
 * S10 UPU: две буквы, девять цифр, две буквы, тринадцать знаков всего. То есть длин ровно три,
 * и **13 из них — самая слабая**: столько же у штрихкода EAN-13 на любой пачке печенья. Поэтому
 * длину правило не ослабляет: 13 цифр пускает не длина, а подпись рядом.
 *
 ...
 *
 * «Рядом» — это **соседние токены**, а не «где-то в тексте»: в строке «Ваша накладна
 * відправлена, а рахунок 1234567890123 сплачено» маркер есть, а номер — счёта. Окно смещено
 * назад ([MARKER_TOKENS_BEFORE] против [MARKER_TOKENS_AFTER]), потому что подпись в документе
 * стоит перед значением: «Експрес-накладна № 0501234567890».
 *
 * **Окно не обрывается переносом строки** (#262, кадр 13). Первая версия считала соседями только
 * токены той же строки — и слепла ровно там, где документ печатают чаще всего: на бумажной
 * экспресс-накладной подпись стоит НАД значением,
 *
 * ```
 * ЕКСПРЕС-НАКЛАДНА №
 * 5900162780858
 * ```
 *
 * а в переписке владельца (`neg_viber`, дословный вывод устройства) номер вообще стоит первым, и
 * «ТТН» подписывает его строкой ниже. Перенос строки в плоском тексте — не смысловая граница, а
 * то, во что OCR превратил вёрстку: требовать от подписи стоять на той же строке значит требовать
 * от вёрстки того, чего она не обещала. Поэтому окно измеряется **в токенах, а не в строках**:
 * на соседнюю непустую строку оно переходит только тогда, когда на своей стороне значения токенов
 * не оказалось вовсе, — то есть остаётся тем же по размеру.
 *
 * **Названная цена.** «Ваша накладна відправлена\n1234567890123» теперь трек, хотя это может
 * быть и номер счёта из следующей строки письма. Улика при этом ровно одна ([formEvidence] даёт
 * [EvidenceClass.SEMANTIC]), человек видит «возможно», а прежняя слепота стоила дороже: на кадре
 * 13 корпуса единственное измеримое действие не ехало вовсе.
 *
...
/** Начало ближайшей непустой строки ниже — та же симметрия, что у окна вперёд на своей строке. */
