...
/**
 * Могло ли тождество ошибиться — и значит, спор ли это (#1303).
 *
 * Ошибиться оно может ровно там, где код страны назвала одна запись из двух: библиотека
 * перечитывает вторую **в стране первой**, снимая её национальный ноль, — и `067 636 05 60`
 * из документа оказывается тем же номером, что и настоящий немецкий `+49 6763 60560`. Страну
 * записи без кода назвать нечем (#1029), различить два случая нечем тоже, — значит слияние
 * спорно, и проигравшее прочтение остаётся при знании.
 *
 * Где кода страны нет ни у одной записи или он есть у обеих, перечитывать нечего: `(918)
 * 682-1561` и `918-682-1561` — одна запись разными знаками, и второй копии значения себе она
 * не заводит (#1011, #932).
 *
 * Страна при этом не называется и не угадывается: спрашивается только, стоит ли в записи код
 * страны вообще.
 */
private fun identityMayBeWrong(key: String, left: String, right: String): Boolean =
    key.removePrefix(META_ENTITY_PREFIX) == "phone" && left.contains('+') != right.contains('+')

/**
 * Проигравшее прочтение остаётся при знании, а не пропадает (#1303).
 *
 * Слияние двух записей одного факта решает, что показать, а не что было прочитано. Список
 * «или» поэтому копит все прочтения, кроме победившего: буквальный повтор в нём не заводится,
 * и своей же копии значение не получает.
 */
private fun keepReading(
    ...
    val kept = (alternativesOf(known, key) + loser)
        .map(String::trim)
        .filter { it.isNotBlank() && it != winner.trim() }
        .distinct()
...
fun mergeFacts(
    ...
    fresh.forEach { (key, value) ->
        ...
        //
        // Проигравшее прочтение при этом не стирается (#1303). Тождество здесь бывает
        // ошибочным: `067 636 05 60` из документа и немецкий `+49 6763 60560` библиотека
        // считает одним номером, потому что вторую запись она перечитывает в стране первой,
        // — а различить два случая можно, только назвав страну записи без кода, что и есть
        // догадка, запрещённая решением по #1029. Значит слияние — спор, и проигравшее
        // остаётся «или», как всякое другое прочтение: молчаливой заменой знание не
        // уничтожается.
        //
        // Экран от этого не шумит: строка «или:» показывает расхождение, а не ту же запись
        // другими знаками (#1011, `otherReading`), и три записи одного номера остаются одним
        // знанием на экране, как их и закрыл #932.
        if (sameFact(key, was, value, region)) {
            val winner = fullerReading(was, value)
            val loser = if (winner == was) value else was
            merged[key] = winner
            if (identityMayBeWrong(key, was, value)) {
                keepReading(merged, known, key, winner, loser)
            } else {
