...
/**
 * Было ли вообще что распознавать (#1023).
 *
 * Человек делится набранным текстом — ни кадра, ни распознавания в этой цепочке нет, и
 * «ошибкам распознавания» взяться неоткуда. Дверь всё равно показывалась (смотрели только на
 * наличие знания), человек нажимал «Исправить ошибки» с пятью опечатками в своём тексте — и
 * читал «Ошибок не нашлось». Работа шла над семью извлечёнными значениями, и узнать это было
 * невозможно.
 *
 * Признак — само знание: хоть одно значение, прочитанное с кадра (#1024). Снимок, у которого
 * знания ещё нет, дверь и так не показывает — исправлять нечего.
 */
fun couldHoldReadingErrors(metadata: Map<String, String>): Boolean =
    metadata.any { (key, value) ->
        key.endsWith(META_SOURCE_SUFFIX) && value == Provenance.OCR.wire
    }
 ...
 *
 * Итог говорит о том, что Point проверял, — о прочитанных значениях (#1023). «Ошибок не
 * нашлось» человек читал как приговор своему тексту, где ошибок было пять, а проверялись
 * семь извлечённых значений, и в них ошибок правда не было.
 */
fun fixedMessage(count: Int): String = when {
    count <= 0 -> "В прочитанном ошибок не нашлось"
