...
interface Realizer {
    ...
    /**
     * Берётся ли эта реализация за **такой** объект (контракт 06.08.2026, И3).
     *
     * Вторая половина вопроса «есть ли чем выполнить». [isAvailable] отвечает про устройство —
     * есть ли пак, ключ, сеть; здесь отвечают про объект. Разделены они потому, что одна и та же
     * способность может быть реализована на двух устройствах по-разному: телефон делает PDF из
     * картинки, текста и офисного документа, а компьютер — только из офисного, настоящим
     * конвертером.
     *
     * Пока этого различия не было, слить такое намерение в общий словарь было нельзя: декларация
     * обещала бы картинку, а исполнить её на компьютере было бы нечем — кнопка, которая откажет.
     *
     * Умолчание `true` — «беру всё, что берёт способность»: у большинства реализаций своего гейта
     * нет, и выдумывать его им незачем.
     */
