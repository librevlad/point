...
data class CapabilityMeta(
    ...
    /**
     * Результат выходит на самом устройстве (#1034).
     *
     * Человек на телефоне нажал «Дать ссылку», согласился выложить файл в интернет, прочитал
     * «Ссылка в буфере — живёт сутки» — и ссылки у него не было: она легла в буфер
     * **компьютера**, потому что для этого действия сильнейшим исполнителем оказался он.
     * Файл при этом сутки открыт любому по ссылке, которой у выложившего нет.
     *
     * Конституция про этот случай прямо: устройство входит в Capability, когда от него зависит
     * внешний результат — «печать на телефоне и печать на компьютере разные действия, потому
     * что бумага выходит в разных местах». Буфер обмена — то же самое: такое действие
     * исполняется там, где человек нажал, и чужому устройству не рекламируется.
     */
    val resultLandsHere: Boolean = false,
)
