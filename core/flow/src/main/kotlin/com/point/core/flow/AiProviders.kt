...
/**
 * Короткий довод для «Недавнего» — ровно на одну строку.
 *
 * Три строки мелким шрифтом делали второстепенную дверь крупнее главной по площади: глаз считает
 * вес пятном, а не свечением, и «Подключить AI» перетягивало «Новый объект» (#580). Ужалось до
 * одной — но ужалось НЕ до перечня молчащего: короткая версия «без ключа молчат „Понять",
 * „Перевести" и „AI"» вернула бы ровно ту беду, ради которой строку когда-то и расширяли —
 * человек читает её первой на пустом доме и делает вывод, что без чужого сервиса Point бесполезен.
 * Поэтому названо работающее, а что добавит ключ — скажет экран, куда строка ведёт.
 */
