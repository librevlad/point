...
/**
 * Короткий довод для «Недавнего»: что молчит без ключа — и что работает и так.
 *
 * Вторая половина здесь не вежливость. Прежний текст называл только молчащее, и человек, читая
 * его первым на пустом доме, делал вывод, что без чужого сервиса Point бесполезен. На деле
 * бесплатно и без сети работает большинство: чтение текста с фотографии, коды, распаковка,
 * превращение форматов — и именно там лежит лучшее, что Point умеет.
 */
const val AI_KEY_WHY_SHORT: String =
    "Чтение текста с фото, коды и распаковка работают и без ключа. Ключ нужен «Понять», " +
        "«Перевести», «AI» и расшифровке записи."
