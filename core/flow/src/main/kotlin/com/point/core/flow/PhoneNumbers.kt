...
object PhoneNumbers {
     ...
     * Взять первую подошедшую страну и назвать её страной номера нельзя (#1029): на этот
     * вопрос отвечает [country], и он молчит, пока стран несколько.
     */
    fun exists(text: String, region: String = DEFAULT_REGION): Boolean =
        candidateRegions(text, region).any { parse(text, it) != null }
     ...
     * сравниваются цифры без кода. Догадка в тождество не входит.
     *
     * Страна устройства отвечает здесь на единственный вопрос — номер ли это вообще (#936);
     * тот же самый вопрос задан при отборе факта.
     */
     ...
     *
     * Формат даёт только известная страна (#1029). Раньше показ разбирал номер **первой
     * подошедшей** страной из списка подсказок: чек из Оклахомы с честным `918-682-1551` в
     * графе выходил на экран как «+49 9186 821551», а на устройстве с американской локалью — как
     ...
     * вовсе на американском. Неизвестная страна не называет и вида.
     */
    fun kind(text: String, region: String = DEFAULT_REGION): String? =
        parsedWithKnownCountry(text, region)?.let {
