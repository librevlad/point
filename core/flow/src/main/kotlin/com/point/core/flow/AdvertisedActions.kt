 ...
 * у него те же — ни одно имя не называет чужое устройство, а каждая строка совпадает со
 * строкой настоящего объявления целиком. Списком, спрятанным в телефоне, его не проверял
 * никто, и он отстал: «Позвонить» и «Создать событие» стояли здесь на любом тексте — с
 * `kinds = TEXT` и без признаков. По этому объявлению компьютер предлагал позвонить тексту
 * без номера и создать событие тексту без даты, а снимку с номером не предлагал ничего.
 * Телефон умеет ровно обратное: номер и дата — признаки, а вид ему безразличен.
 */
val PHONE_ADVERTISED_FALLBACK = listOf(
    PcRemoteAction("call", "Позвонить", features = setOf("HAS_PHONE"), priority = 40),
    PcRemoteAction("event", "Создать событие", features = setOf("HAS_DATE", "IS_MEETING"), priority = 16),
)
