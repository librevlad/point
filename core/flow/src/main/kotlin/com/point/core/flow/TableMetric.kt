 ...
 * точки, **стиль кавычки** и пунктуация по краям — «120.» и «120» человек читает одинаково.
 *
 ...
 *
 * Края — по замеру кадра 06 (окно учётной программы): вёрстку окна у номера договора движок
 * читает прилипшей точкой или скобкой — «.286/2/18/138», «(286/2/18/137», «.286/2/20/3.» — и
 * метрика объявляла пять строк потерянными и пять лишними на начисто прочитанной таблице,
 * то есть винила чтение за вёрстку окна. Складывается только край ([EDGE_NOISE]): ведущий
 * ноль — значение (сколько разрядов значащих, знает документ, — решение при
 * [meterWithoutDrumZeros]), внутренняя точка — тоже («1.5» ≠ «15»). Испорченный ключ краем
 * не спасается: «128», прочитанный как «138», — по-прежнему потерянная строка плюс лишняя.
 */
...
/** Точка и скобка, прилипшие к краю значения, — вёрстка бланка или окна, не значение. */
