...
enum class TableFailure(val reason: String) {
    ...
    SILENT_CELLS("значение разошлось с эталоном, и ничто об этом не сказало"),
    LOST_ROWS("строка документа в файл не попала"),
    EXTRA_ROWS("в файле есть строки, которых в документе нет"),
    WARNING_WALL("⚠ стоит на трети ячеек и больше — человек перепроверяет таблицу целиком"),
...
data class TableScore(
    ...
    /** Чем провалено — в порядке важности. Пустой список = таблица сдана. */
 ...
 * [styleCell]), регистр, любые пробелы (включая разрядный в «1 200») и десятичная запятая против
 * точки. Хвостовая точка снимается — «120.» и «120» человек читает одинаково.
 */
