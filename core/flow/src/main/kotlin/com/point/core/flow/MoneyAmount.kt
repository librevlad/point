 ...
 *
 * Пометка валюты стоит **при** числе: знак — с любой его стороны, слово — после числа.
 * Какое это слово, судья формы не спрашивает: «1200 kr», «1200 лв», «500 гривень» и
 * «1200 dollars» — такие же числа, как «$2.18», и никакой список валют не вправе выбрасывать
 * их из знания. Граница у пометки одна и названа в [CURRENCY_TAIL]: пометка — одно слово,
 * а не фраза.
 ...
 *
 * Здесь и живёт сильный вопрос о подписи: **какой именно** суммы. Улика подписи ([namesAmount])
 * его не задаёт и задавать не должна — она отделяет строку с подписью от строки без подписи.
 *
 ...
 *
 * Их же берёт судья улик (`FIELD_MARKERS`), и читает он их тем же правилом, что и правило
 * страницы, — [namesAmount]. Валюты в этом списке нет: знак и слово валюты стоят при **каждой**
 * сумме документа, поэтому улику подписи получала и строка полученных денег.
 */
internal val AMOUNT_MARKERS = TOTAL_MARKERS + listOf("сума", "сумма", "amount")

/**
 * Называет ли подпись сумму документа (#1059) — вопрос судьи улик.
 *
 * Вопросов здесь два, и путать их нельзя. Судья спрашивает слабый: **названа ли строка суммой
 * вообще**, — потому что улика подписи отделяет строку с подписью от строки без подписи, и
 * больше ни от чего. Какой именно суммы — итога, скидки или налога — спрашивает [mainAmount]
 * уже среди равных по уликам, сильным [labelledTotal].
 *
 * Прежде судья спрашивал здесь сильный вопрос — «названа ли строка итогом и ничем иным», — и
 * подпись из двух слов не давала улики вовсе: «TOTAL DUE», «AMOUNT DUE», «Сумма перевода» не
 * были подписаны ничем. На слое слов «Сумма перевода 1 000,00 / Остаток 25 000,00» улик
 * выходило поровну, и главной суммой вставал остаток по счёту — деньги, которых человек
 * никуда не переводил.
 */
internal fun namesAmount(label: String): Boolean = namesTotal(label, head = false, AMOUNT_MARKERS)
...
// Разряды пробелом — как раньше; плюс однозначная запись разрядов точкой или запятой, когда
// копейки стоят после ДРУГОГО разделителя: «1.234,56» и «1,234.56» читаются, а одиночная
// «1.234» — нет, иначе «3.14» стало бы 314 (решение владельца #1440: только однозначные).
...
/**
 * Пометка валюты после числа у судьи формы (#1059): знак или слово.
 *
 * Здесь значение уже названо суммой — и вопрос один: число ли это. Какими буквами написана
 * валюта, судья не спрашивает: «kr», «Kč», «лв», «500 гривень», «1200 dollars» — деньги ровно
 * настолько же, насколько «JPY» и «грн», а список валют, решающий за них, отнимал у человека
 * знание вовсе. Прежде здесь стояла длина — не больше четырёх букв, — и она резала знание тем
 * же самым образом: «20 евро» проходило, «100 долларов» нет.
 *
 * Граница есть, она одна, и вот она: пометка — **одно слово**, а не фраза. Поэтому целая
 * строка чека «2.18 CASH TOTAL» суммой не станет. Какое это слово, судья не спрашивает:
 * «20842 кВт» и «2.18 TOTAL» он пропустит суммой — так же, как пропускал до #1059, когда
 * считал одни цифры. Это гейт «число ли это», а не «какая это валюта»; живой дефект
 * карточки — буквы **перед** числом — закрыт отдельно.
 */
private const val CURRENCY_TAIL = "(?:\\p{Sc}|\\p{L}+\\.?)"
