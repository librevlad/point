...
data class CropEvidence(
    ...
    val region: Box,
    /**
     * На сколько повернуть вырезанное, чтобы оно встало так же, как кадр, который видел человек.
     *
     * Область режется из файла как есть, а файл камера могла записать повёрнутым. Значения по
     * умолчанию у этого числа нет намеренно (#1389): пока оно было, «Взять фрагмент» его не назвал
     * и молча получил ноль — человек выделял ровный текст и получал картинку на боку.
     */
    val uprightDegrees: Int,
    val purpose: CropPurpose = CropPurpose.GLANCE,
)

/**
 * На сколько развёрнут кадр в файле по метке камеры.
 *
 * Спрашивается у того, кто открывает файл. Считать это число заново своей копией той же логики
 * нельзя: своя копия про метку и забывает, а вырезка тогда встаёт не так, как виденный кадр.
 */
fun interface UprightAngle {
    fun degreesOf(path: String): Int
}
