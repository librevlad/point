...
/**
 * Как называется дверь, за которой лежит ключ, — одним словом на всех (#544).
 *
 * Отказ обязан звать человека туда, что он увидит глазами. Пока слово было написано в каждом месте
 * отдельно, оно и разъехалось: дверь на «Недавнем» звалась «AI-ключ», отказ звал к «шестерёнке на
 * домашнем экране» — которой не было с #462, — а экран за дверью назывался «Настройки». Три имени
 * одной вещи, и два из трёх человек найти не мог.
 *
 * Поэтому имя одно и живёт здесь, ниже всех: его читают дверь (`:app`), заголовок экрана (`:app`) и
 * отказ из цепочки моделей (`:data`). Разъехаться им больше негде.
 */
