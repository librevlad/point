...
interface Capability {
    ...
    /**
     * Почему действие сейчас не сработает, хотя оно на месте и нажимаемо (#1022).
     *
     * Не то же, что [missing]: там названо, чего не хватает, чтобы действие вообще
     * предлагалось, и человек читает это подсказкой. Здесь действие видно и выбрано — и по
     * тапу человек слышит причину вместо экрана согласия: спрашивать разрешение на то, что
     * всё равно не выйдет, незачем (линза #1003). Прятать действие при этом нельзя —
     * человек не увидит, чего ему не хватает.
     */
    fun wontWorkNow(state: ObjectState): String? = null
}
