...
interface Capability {
    ...
    /**
     * **Что вернётся, если нажать** (#491) — второе, что человек имеет право знать до тапа, после
     * имени действия. «Вернёт текст», «вернёт таблицу», «ничего не вернёт — отправит».
     *
     * Отдельно от [produces], потому что [produces] на два разных ответа даёт одно значение:
     * терминальное действие возвращает свой аргумент, и действие, которое **обогащает тот же
     * объект**, — тоже («Понять» дописывает факты и отдаёт его же). Человеку это ровно та
     * разница, ради которой он и читает строку.
     *
     * Умолчание выводится из [produces] и различает то, что различимо: `null` → неизвестно (AI),
     * **тот же объект по ссылке** → ничего не вернёт, иначе → новый объект такого вида. Именно по
     * ссылке, а не по значению: «Скан» отдаёт `ObjectState(IMAGE)` с картинки без признаков —
     * значение равно входному, а объект будет новый. То же правило, которым [intents] отличает
     * терминальное действие от превращения в свой же вид.
     *
     * Переопределяют в двух случаях: умолчание врёт (объект тот же, но понятый — [ActionYield.Same])
     * либо вид слишком широк, чтобы называть его человеку (`OFFICE` — и документ, и таблица).
     *
     * Это **ожидание, а не обещание**: истинный вид переклассифицируется из реального выхода, и
     * разошедшийся исход досказывается словами (`yieldSurprise`), а не заминается.
     */
