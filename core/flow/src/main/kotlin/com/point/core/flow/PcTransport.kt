...
/**
 * Сколько попросивший ждёт ответа соседа, прежде чем уйти (#672, #1321).
 *
 * Одно число на обе стороны связки — тем же правилом, что и `CAPS_FRESH_MS`: по нему
 * попросивший перестаёт ждать ответа, и по нему же тот, кого попросили, понимает, что
 * отвечать уже некому, — и отправляет исход очередью. Порознь эти два срока разъехались бы
 * молча, и исход снова уходил бы кадром тому, кто не спрашивает.
 */
const val PC_ANSWER_WAIT_MS: Long = 25_000
