...
/** Дорога ответа обратно: положить в ящик и дождаться, пока его оттуда спросят (#1321). */
private const val ANSWER_TRIP_MS: Long = 3_000
...
/**
 * До какого возраста просьбы срочный ответ ещё застаёт попросившего (#1321).
 *
 * Попросивший ждёт `PC_ANSWER_WAIT_MS` с того мига, как отправил письмо, и уходит. Ответ идёт
 * к нему не мгновенно: его надо положить в ящик, а ящик попросивший спрашивает раз в секунду
 * (`RelayRpcClient.pollMillis`), — поэтому у ответа, собранного впритык к сроку, застать уже
 * некого, и запас на эту дорогу входит в порог.
 *
 * Ошибиться здесь можно в две стороны, и они не равны. Сочтя ушедшего слушающим, исход теряют
 * совсем: срочный ответ уедет кадром в никуда, а в очередь его уже не положат. Сочтя
 * слушающего ушедшим, человек несколько секунд видит «ещё работает» и получает исход
 * очередью. Поэтому запас берётся в сторону очереди.
 *
 * Порог выведен из самого срока и записан один раз. Сросшись вместо этого с бюджетом ответа,
 * он переворачивался бы от правки чужого числа: стоит бюджету дорасти до срока — и срочный
 * ответ пропадает разом для всех просьб, включая живые.
 */
const val PC_ANSWER_HEARD_MS: Long = PC_ANSWER_WAIT_MS - ANSWER_TRIP_MS
