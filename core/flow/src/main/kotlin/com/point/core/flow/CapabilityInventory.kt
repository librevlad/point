...
/**
 * Приписка, которой действие договаривает цену прямо в собственном имени (#529).
 *
 * Одно написание на всех: пока фраза жила строкой внутри расшифровки, остальные семь действий с
 * `meta.auth` молчали, и человек узнавал про ключ после минуты ожидания. Слово ровно одно и то же,
 * чтобы «нужен ключ» на экране всегда значило одно и то же.
 */
const val KEY_NOTE: String = "нужен ключ"

/**
 * Имя действия, которое без ключа не умалчивает о том, что ключ нужен (#529).
 *
 * Действие при этом **не прячется и не блёкнет**: оно есть, оно про этот объект, и человек имеет
 * право по нему тапнуть — тап откроет экран ключей. Меняется только имя, и только в ту сторону,
 * где сказано больше правды.
 *
 * Какой именно ключ — говорит отказ после тапа: коротко на строке этого не сказать, а полуправда
 * («нужен ключ Groq») отправила бы человека чинить то, что чинится и иначе.
 *
 * [keySet] спрашивают у контракта готовности ([AiReadiness], [SpeechReadiness]) — у той цепочки,
 * которой это действие живёт. Общего ответа тут быть не может: у расшифровки свои ключи, и
 * приписка «нужен ключ» на живом движке была бы свежей неправдой вместо починенной старой.
 */
