...
/** Приписка целиком — то, что [labelNeedingKey] дописывает к имени и что читают обратно. */
private const val KEY_NOTE_SUFFIX = " · $KEY_NOTE"

/**
 * Действие само сказало в своём имени, что сейчас без ключа не сможет (#465).
 *
 * Признак читается с той же стороны и той же константой, которой он пишется ([labelNeedingKey]):
 * второй правды о том, «нужен ли ЭТОМУ действию ключ», не заводится. Спрашивать вместо этого
 * `meta.auth` было бы неверно — объявленный `auth` есть и у «Прочитать сильнее», которое в
 * раздаваемой сборке читает страницы без единого ключа (#529). Право сказать «мне нужен ключ»
 * есть только у самой способности, и она уже сказала это своим именем.
 */
fun labelNeedsKey(label: String): Boolean = label.endsWith(KEY_NOTE_SUFFIX)

/** Как действие зовут, когда ключ есть: имя без приписки — «Понять», а не «Понять · нужен ключ». */
fun labelWithoutKeyNote(label: String): String = label.removeSuffix(KEY_NOTE_SUFFIX)

/**
 * Зачем ключ ИМЕННО этому действию (#465) — первое, что человек читает, свернув за ключом.
 *
 * Общего «AI не настроен» здесь мало: человек шёл не настраивать AI, он тапнул «Понять» на своей
 * фотографии. Строка называет его действие по имени, говорит, чем оно живёт, и обещает конец пути
 * — возврат к объекту. Обещание держит не текст, а [labelWithoutKeyNote] на том же экране: имя
 * приходит из самой строки действия, а не переписывается здесь руками.
 */
...
/**
 * Чем вышедшее оказалось **по существу** — тем же словом, каким это обещали до тапа (#558).
 *
 * Ключ метаданных результата. Ставит его реализатор, который знает, что на самом деле сделал;
 * читает — [yieldSurprise], сверяя с обещанием способности. Без ключа сверять нечего, и это
 * законное умолчание: у большинства превращений вид и есть существо.
 *
 * Заведён по жалобе владельца «Word в PDF молча дал не то, что человек хотел»: офисный файл
 * превращается в PDF **пересказом**, вид совпадает с обещанным — и молчания было не пробить,
 * пока сторож сравнивал только вид.
 */
const val META_YIELD_NOUN = "yield.noun"

/**
 * Вышло не то, что было обещано, — и это сказано вслух (#491, по существу — #558).
 *
 ...
 *
 * Сверяются **две вещи, и вторая важнее первой**:
 * 1. вид — «ожидался текст, вышел документ»;
 * 2. существо ([actualNoun] против слова обещания) — вид совпал, а внутри другое. Именно так
 *    молчал «Word в PDF»: настоящий PDF, в котором вместо документа его пересказ.
 *
 * Приписка про цену («текст · снимок уйдёт в сервис») в сверку не идёт: она про дорогу, а не про
 * то, что вышло, и сравнивать по ней значило бы ловить расхождение там, где его нет.
 *
 * `null` — расхождения нет либо судить не о чем ([ActionYield.Unknown] у AI ничего не обещал,
 * [ActionYield.Same] и [ActionYield.None] судятся не видом, а наличием объекта; молчащий
 * [actualNoun] значит «реализатор о существе ничего не сказал», а не «всё сошлось»).
 */
...
/** Существо без приписки про цену: «текст · снимок уйдёт в сервис» → «текст». */
