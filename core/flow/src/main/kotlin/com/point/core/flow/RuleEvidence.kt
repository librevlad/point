 ...
 *   (живой случай #244, где время статуса вытесняло настоящую дату); сюда же формы трека,
 *   которые печатаются одним куском ([looksLikeTrackToken]: `5900162/7808586`, S10).
 *
...
fun AtomLayer.ruleEvidence(): Map<String, List<String>> {
    ...
    lines(atoms.filter { it.text.isNotBlank() }).forEach { line ->
        line.forEach { atom ->
            ...
            // Формы, которые целиком помещаются в один атом (#262): напечатанный через косую
            // номер накладной и S10 с сошедшейся контрольной цифрой. Пробеговое правило их не
            // видит вовсе — «5900162/7808586» не цифровой пробег, — а подсказка модели нужна
            // ровно та же, что у 14 цифр.
