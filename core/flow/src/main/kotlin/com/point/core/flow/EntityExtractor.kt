 ...
 * «Дата», вытесняя настоящую дату, которая на том же кадре есть. Календарная часть — то, что
 * отличает дату документа от отметки времени, поэтому правило требует её наличия, а не
 * отсутствия чего-то.
 *
...
fun Entity.isPlausible(): Boolean = when (type) {
    ...
    // Голое время суток — не дата документа: см. [BARE_CLOCK].
    EntityType.DATE_TIME -> !BARE_CLOCK.matches(value.trim())
    else -> true
}
