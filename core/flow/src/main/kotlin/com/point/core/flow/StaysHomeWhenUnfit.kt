 ...
 *
 * Исполнитель объявляет себя внешним сам (`RealizerKind.CLOUD`). Но у части сетевых
 * способностей — «Понять», «Перевести», AI, «Дать ссылку» — единственный исполнитель
 * почему-то назван местным, хотя внутри зовёт модель или отдаёт байты наружу. Способность
 * здесь надёжнее: `CapabilityMeta.network` объявлен именно там и не разъезжается с
 * реализацией так, как это уже случилось однажды с `RealizerKind`.
 */
fun sendsOutward(realizer: Realizer, capabilityIsNetwork: (CapabilityId) -> Boolean): Boolean =
    realizer.meta.kind == RealizerKind.CLOUD || capabilityIsNetwork(realizer.capabilityId)
