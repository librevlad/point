...
/**
 * Крошечный читатель JSON — ровно столько, сколько нужно, чтобы понять ответ сервера Point (#472).
 *
 * Зачем свой, а не готовый. `:core:flow` держится **без единой зависимости** — в этом смысл модуля:
 * его код одинаково едет и в APK, и в MSI, и тестируется на голой JVM. `org.json` есть только на
 * Android, у Compose Desktop его нет вовсе; тянуть парсер в оба артефакта ради четырёх плоских
 * ответов — плата больше выигрыша. Здесь читаются объекты, массивы, строки, числа, `true/false/null`
 * — и больше ничего: ни своих расширений, ни хитростей. Незнакомое или битое даёт `null`, а не
 * исключение наружу: ответ сервера — чужой текст, и падать на нём нельзя.
 *
 * Писать JSON нечем намеренно: наружу уходят два-три поля, и [jsonObject] собирает их строкой с
 * честным экранированием — своего сериализатора объектов проект не заводит.
 */
...
/** Разобрать текст; `null` — это не JSON или он оборван (чужой текст падать не имеет права). */
