...
data class Bubble(
     ...
     * так отмечен. Дверь, берущая объект как есть, остаётся в списке: причина становится
     * подписью там, где второй строки обычно нет (#582), а не прячет действие (решение
     * владельца — «дверь не исчезает»). Двери чтения негодному не предлагаются вовсе (#994).
     */
