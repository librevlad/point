...
/**
 * Выбор знака объекта (#295) — чистая логика, без Compose и без устройства.
 *
 * Тест сторожит ровно две вещи, которые легко потерять при следующей правке: знак носит тот, кто
 * ЕСТЬ таблица (вид объекта решает, MIME только уточняет), и .xlsx узнаётся даже когда Share
 * привёз его безымянным типом — так реально приезжают OOXML-файлы, это zip.
 */
class SpreadsheetMarkTest {
    ...
    fun `xlsx, приехавший безымянным типом, узнаётся по расширению`() {
        // Так его отдаёт добрая половина файловых менеджеров — OOXML это zip.
        assertEquals(
    ...
    // --- фазы рождения знака (MOTION.md принцип №1) ---
    ...
    fun `знак Excel загорается последним — позже, чем наполняются строки`() {
        // Смысл анимации именно в порядке: сначала данные легли, потом это стало Excel.
