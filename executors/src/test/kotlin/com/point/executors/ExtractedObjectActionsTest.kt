...
/**
 * Actions performed **on an extracted object** (#222, шаг 4 → #237).
 *
 * The sprint claimed that tapping a found address gives «Открыть на карте» with no new code. Half
 * of that was true and the half nobody checked was false: `accepts` looks at the feature and lit
 * the bubble, `perform` went looking for a file and found none, because an extracted object's ref
 * is its value. The bubble appeared, the tap failed. A bubble is a flag, not a result.
 *
 * These tests act on the object the way the user does — by tapping the thing itself.
 */
class ExtractedObjectActionsTest {
    ...
    private fun found(kind: ObjectKind, value: String, feature: Feature) = PointObject(
        id = "src:found",
        mime = "text/plain",
        uri = ValueRef(value), // no file behind it — this is the whole point
    ...
    /** An extractor that finds nothing: a fifteen-character value is not what ML Kit was
     *  trained on, and the object must not depend on it succeeding. */
    ...
    fun `the value is read from the object, not rediscovered by a model`() = runTest {
        // A model that would "find" something else must not be able to override what the object
        // already is. The object knows its own value; asking again can only make it worse.
    ...
    fun `a file-backed object still reads its file`() = runTest {
        // The fix must not cost the ordinary path: a TEXT object keeps working through the
