...
/**
 * Результат возвращается туда, где его попросили (контракт 06.08.2026).
 *
 * До этого работа кончалась словом. Человек просил с телефона распознать снимок, получал «готово»
 * — и шёл за текстом к компьютеру. Формула продукта одна на оба устройства: объект, действие,
 * снова объект; последней части здесь не было вовсе.
 *
 * Это не автоматическая цепочка. Человек сделал один явный выбор, и возврат результата — не
 * следующий шаг, а окончание этого: разница та же, что между «отправил письмо» и «получил ответ на
 * своё письмо».
 */
class ResultComesBackTest {
    ...
    /** Компьютер, который делает работу и отвечает тем, что получилось. */
    ...
    @Test fun `компьютер вернул текст — он становится объектом здесь, а не остаётся там`() = runTest {
        ...
        // Понимание возвращается вместе с байтами: объект приходит разобранным, а не голым файлом.
        assertEquals("4512", born.metadata["entity.amount"])
    }

    @Test fun `компьютер вернул только слово — говорим словом, как раньше`() = runTest {
        // Так отвечает доставка («напечатал») и старая сборка компьютера, которая объектов
        // возвращать не умела. Выдумывать за неё объект нельзя.
        val realizer = RemotePcRealizer(
    ...
    @Test fun `без приёмника объект не выдумывается — человек читает прежние слова`() = runTest {
        // Приёмника нет только в сборках, где реализатор собран без него. Молчание честнее
        // объекта, который некуда положить.
        val realizer = RemotePcRealizer(
            PcRemoteAction("ocr", "Распознать текст"),
            Linked,
