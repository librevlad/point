...
class AmountIsANumberTest {
     ...
     *
     * Что строки этого слоя именно такие, проверяет тест ниже, а не эта запись: сами подписи
     * стоят на разной высоте, и по их координатам «TOTAL» — отдельная строка над «CASH».
     */
    ...
    fun `слой слов даёт те же строки чека, что названы в карточке`() {
        // Строку слоя решает близость середин: приходящее слово встаёт в текущую строку, если
        // от середины её последнего слова его середина отстоит не больше чем на половину его
        // собственной высоты (`AtomLayer.rowsOf`). Сами подписи так стоят врозь: середина
        // «TOTAL» — 384.8, «CASH» — 413.7, это 29 при половине высоты «CASH» 13.8, и слой дал
        // бы две строки. В одну их сводит третье слово — тот самый слипшийся ноль `w10`: его
        // коробка высотой 238 растянута по всей колонке чисел, середина 406.1 попадает между
        // подписями, и цепочка смыкается через него. Убрать `w10` — и слой даёт «TOTAL» и
        // «CASH» отдельными строками, то есть эта проверка упала бы.
        //
        // Значит согласие двух следов чтения держится на одном слове уверенности 0.20, а не
        // на свойстве строки. Это и есть слипание, названное в карточке: ноль размазан на всю
        // колонку сумм — он и склеил строки чека, из-за которых число оторвалось от подписи.
