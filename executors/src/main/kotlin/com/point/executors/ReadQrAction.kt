...
/**
 * image with a QR → its contents as text (#85 transform). Lit up only once [QrEnricher] confirms a
 * QR is present. The result is a TEXT object; if it holds a URL the enrichers then offer "Открыть
 * ссылку", so a screenshot's QR becomes a tappable link.
 */
