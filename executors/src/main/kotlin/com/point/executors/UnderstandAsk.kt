...
/**
 * Point говорит с человеком по-русски — и знание об объекте тоже (#670).
 *
 * Правило жило в тексте одного промпта из двух, а во втором стояло прямо противоположное
 * («на языке документа»), и подзаголовком объекта у русскоязычного человека вставало
 * `Family Dollar store receipt for drinks` — в зависимости от того, какой путь понимания
 * сработал (#1036). Теперь правило одно на все пути: где просят SUMMARY, туда идёт эта строка.
 */
internal const val ANSWER_IN_RUSSIAN =
    "SUMMARY и любые словесные значения пиши по-русски, даже если сам документ на другом языке. "
...
internal fun understandPrompt(
    ...
    append(
        "Если ты не уверен, каких кандидатов на поле несколько — перечисли до " +
            ...
            "Добавь строку SUMMARY=<суть текста в 3-6 словах>. " + ANSWER_IN_RUSSIAN + "\n\n",
    )
