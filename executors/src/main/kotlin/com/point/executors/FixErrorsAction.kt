...
internal suspend fun fix(llm: LlmClient, input: PointObject, withObject: Boolean): ActionResult =
    withContext(Dispatchers.IO) {
        ...
        runCatching {
            ...
            // Гейт формы тот же, что у находки (#666, #1032), и страница ему нужна та же:
            // слово-подпись накладной стоит в прочитанном тексте. Текст объекта на этом пути
            // не правится — правится значение, — поэтому страницу под исправленное значение
            // готовит сам разбор ответа.
