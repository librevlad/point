 ...
 *
 * METER и GEO — #262, тем же порядком: у обоих есть офлайновое правило формы ([meterFacts],
 * [geoFacts]), и оно остаётся бесплатным первым источником, но слепо к тому, чего не знает, —
 * показанию без единицы рядом (табло водомера на фото) и координатам в градусах-минутах.
 * Два независимых источника одного факта встречаются голосованием ([mergeFacts]), а не
 * подменяют друг друга.
 *
 * AMOUNT и RECEIPT — то же самое для денег (#262): правило формы у обоих есть ([amountFacts],
 * [receiptFacts]). Сумму внутри расчёта «127*4.32=548,64» правило с тех пор читает само —
 * итогом сошедшегося расчёта ([arithmeticTotals]), — но слепым оно быть не перестало: цену от
 * суммы к переводу форма не отличает, а расчёт не знает, что он про деньги. Второй читатель
 * поэтому остаётся, и встречаются они голосованием. SUBJECT правила не имеет **никакого**: тема
 * письма отличается от прочих строк не написанием, а местом на экране, — тот же случай, что у
 * PLACE, и потому единственный писатель у неё модель.
 */
...
class UnderstandRealizer @Inject constructor(
        ...
        withContext(Dispatchers.IO) {
            runCatching {
                ...
                if (fields.isEmpty() && parsed.single.isEmpty() && roles.isEmpty()) {
                    // #526: чтение состоялось и ничего нового не принесло — это нормальный исход,
                    // а не поломка. Отказом («Ничего нового не найдено») экран красил его тревожным
                    // знаком, и человек читал «сломалось» там, где Point честно прочитал всё, что
                    // было, и ничего не потерял: объект остаётся на экране со всем, что о нём уже
                    // известно. Терминальность здесь настоящая — нового объекта у шага нет.
    ...
    private companion object {
        ...
        /** Исход «нового нет» словами человека, а не отчётом о пустом результате (#526). */
