 ...
 *
 * Не срыв: читатель ответил. Срыв оставил бы вопрос не исследованным (ADR-0001 §9). Но и не
 * последнее слово: в очереди исполнителей такое «не нашлось» уступает ход следующему
 * ([FallbackRealizer]) — пустой ответ одного читателя не закрывает вопрос за тех, кто видит
 * сильнее; ответом оно остаётся, когда посмотрели все.
 */
