...
class CloudOcrCapability @Inject constructor() : Capability {
    ...
    //
    // #529 сюда НЕ приходит, хотя `meta.auth` объявлен: приписки «нужен ключ» здесь не будет.
    // Цепочка начинается с внешнего глаза (`ExternalEyeCloudOcrRealizer`), а две из трёх его
    // ручек работают без регистрации — в раздаваемой сборке без единого ключа это действие
    // читает страницы и читает хорошо. Приписка была бы не починенной ложью, а новой: человек
    // шёл бы заводить ключ ради того, что и так работает. Ключ нужен только последнему звену,
    // и когда до него дойдёт — скажет оно само, своим отказом.
