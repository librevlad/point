...
internal const val DEEP_UNDERSTAND_PROMPT =
    // #236: текст приходит из распознавания фото и бывает повреждён — «Олексіївка» читается как
    // «Олексйвка». Модели разрешено чинить такие искажения и достраивать значение до полного, но
    // не выдумывать: цифры она не правит вовсе, а правка текста ограничена расстоянием
    // (isRepairOf) — далёкая «правка» считается разногласием и уходит в голосование.
    "Текст распознан с фотографии и может содержать ошибки распознавания. " +
        "Найди в нём контактные данные. Значение приводи ПОЛНОСТЬЮ, как оно есть в документе " +
        "(адрес — вместе с населённым пунктом), и исправляй только явные искажения распознавания. " +
        "НИЧЕГО не додумывай: если чего-то в тексте нет — не пиши строку. Цифры не меняй. " +
        "Отвечай ТОЛЬКО строками вида " +
