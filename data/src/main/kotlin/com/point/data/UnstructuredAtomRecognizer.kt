...
/**
 * Unstructured — бесплатный читатель структуры документа (~15 000 страниц/мес, ключ выдаётся без
 * привязки карты). Разбирает страницу на элементы (заголовок, абзац, таблица) и при
 * `coordinates=true` отдаёт каждому многоугольник на странице.
 *
 * Контракт запроса (проверено по документации на 2026-08): `POST` на
 * `https://api.unstructuredapp.io/general/v0/general`, ключ в заголовке `unstructured-api-key`,
 * тело — `multipart/form-data` с полем `files`. Ответ — **массив** элементов вида
 * ```json
