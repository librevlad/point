...
/**
 * Выбранный уровень «куда можно отправлять» — в prefs, как соседние настройки.
 *
 * Хранится **именем перечисления**, а не порядковым номером: вставь завтра четвёртый уровень в
 * середину — и у всех, кто уже выбрал, настройка молча съехала бы на соседнюю. Молчаливая смена
 * настройки приватности — худший сорт сбоя, потому что человек её не увидит.
 *
 * Сбой чтения prefs — это [PrivacyLevel.DEFAULT], а не «закрыть всё»: тихо ужесточить настройку
 * значило бы сломать распознавание, не сказав ни слова.
 */
@Singleton
