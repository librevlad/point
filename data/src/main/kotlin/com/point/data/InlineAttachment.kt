...
/**
 * Байтовый бюджет кадра: тяжелее — перекодируем, даже если по пикселям он в пределе (скрин таблицы
 * на 9 МБ ужимается в разы без единого потерянного пикселя разметки).
 *
 ...
 * (JPEG положил бы вырезанный объект #97 на чёрный фон), а не забытая ветка, — но экономию по
 * байтам гарантирует только пиксельный предел [MODEL_MAX_EDGE_PX], не бюджет.
 */
