...
class GroqWhisperSpeechToText(
    ...
    override suspend fun transcribe(obj: PointObject): Transcription = withContext(Dispatchers.IO) {
        ...
        val res = http.postMultipart(
            ...
            headers = mapOf(
                ...
                // ЭТОГО сервиса он обязателен: без него Groq отвечает 403, и провайдер уже однажды
                // числился из-за этого мёртвым. Названный явно — он судится тестом рядом.
    ...
    private fun refusal(code: Int, body: String): String = when (code) {
        ...
        // Мы его шлём, но если ответ всё-таки 403 — человеку названы обе причины, а не одна.
        403 -> "Whisper - Groq отказал (403): ключ не принят либо запрос ушёл без имени"
