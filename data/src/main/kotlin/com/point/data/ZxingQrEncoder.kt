...
/**
 * On-device QR encoding via ZXing (pure Java → [BitMatrix]) rendered to a PNG. UTF-8 char set so
 * Cyrillic survives; error-correction M is the readable/robust sweet spot. Writes into the scratch
 * store and returns the ref — so the realizer stays thin. ZXing throws for text past QR capacity;
 * that surfaces as a recoverable failure upstream.
 */
