...
class QrInvestigation @Inject constructor() : Capability {
    ...
    override val meta = CapabilityMeta(
        ...
        // Мерка стоимости — работа исполнителя, а не объявленная лёгкость (#1240). Взгляд на
        // кадр раскодирует картинку целиком, а самая дорогая ветка — именно «не нашлось»:
        // после первого сканера кадр декодируется второй раз, внимательнее. Названная цена
        // включает гейт по знанию: отвеченный вопрос при повторном входе в тот же объект не
        // задаётся заново. Дешёвое переспрашивать по-прежнему можно (#669) — это не про него.
