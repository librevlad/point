...
/**
 * Компьютер, о котором телефон знает, — тот же крошечный тёплый JSON-файл, что у [FileChosenApps].
 *
 * Правда о круге живёт на сервере, здесь — её последний слепок. Он нужен ровно затем, что первый
 * экран обязан уложиться в 300 мс без сети: пузырёк «На компьютер» решает, показываться ли, по
 * этой записи, а не по запросу.
 *
 * Файл называется по-старому (`pc-pairing.json`) намеренно: у тех, кто уже поставил сборку,
 * лежит запись про пару, и она честно не читается — полей нового вида в ней нет. Заводить второе
 * имя ради того же смысла значило бы оставить на диске мусор, который никто не уберёт.
 */
@Singleton
