...
/**
 * Reads an OOXML `.xlsx` back into rows — the inverse of [OoxmlSpreadsheetWriter]. Handles the
 * three ways a cell holds text: inline strings (what our writer emits), the shared-strings table
 * (what Excel/Sheets emit) and bare numeric values. Dependency-free (zip + regex), unit-tested.
 *
 * Fixes «xlsx → PDF»: our own writer stores inline strings with no `xl/sharedStrings.xml`, which
 * the text extractor could not read — so converting Point's own spreadsheet dead-ended.
 */
class OoxmlSpreadsheetReader @Inject constructor() : SpreadsheetReader {
    ...
    /** The shared-strings table: one entry per `<si>`, its runs' `<t>` concatenated. */
    ...
    private fun parseRow(rowXml: String, shared: List<String>): List<String> {
        ...
        for (cell in CELL.findAll(rowXml)) {
            ...
            val body = cell.groupValues[2] // "" for a self-closed <c .../>
    ...
    /** `r="AB12"` → 0-based column index (A→0 … Z→25, AA→26 …). */
