...
class GraphRolesInvestigationTest {

    private val enricher = GraphRolesInvestigationRealizer()
    ...
    fun `a classified sender becomes an Organization with the sender relation`() = runTest {
        val delta = enricher.look(doc("sender" to "ТОВ «Агротрейд»"))
    ...
    fun `записанное происхождение сильнее фолбэка — правку человека модель не переписывает`() = runTest {
        ...
        val org = enricher.look(edited).objects.single()
    ...
    fun `спор об имени роли виден и на самом узле, а не только на документе`() = runTest {
        ...
        val node = enricher.look(disputed).objects.single()
    ...
    fun `one organisation in two roles is one node with two relations`() = runTest {
        ...
        val delta = enricher.look(doc("carrier" to "Нова Пошта", "issuer" to "Нова Пошта"))
    ...
    fun `spacing and case do not split one company into two`() = runTest {
        val a = enricher.look(doc("sender" to "Нова  Пошта")).objects.single()
        val b = enricher.look(doc("carrier" to "нова пошта")).objects.single()
    ...
    fun `a blank role value is not a node`() = runTest {
        assertTrue(enricher.look(doc("sender" to "   ")).objects.isEmpty())
    }
    ...
    fun `an organisation is not classified out of again`() = runTest {
        ...
        assertTrue(enricher.look(org).objects.isEmpty())
    }

    @Test
    fun `re-running after a restart rebuilds the same graph, asking the model nothing`() = runTest {

        val first = enricher.look(doc("receiver" to "ТОВ «Агротрейд»"))
        val again = enricher.look(doc("receiver" to "ТОВ «Агротрейд»"))
