...
class LlmSpeechToTextTest {
    ...
    @Test
    fun `суть записи просят по-русски, а слова — дословно (#1036)`() = runTest {
        // Суть записи ложится подзаголовком тем же ключом, что суть снимка или текста, а
        // просилась «на языке записи» — украинское голосовое давало украинский подзаголовок,
        // пока снимок давал русский. Правило языка одно; дословные слова записи оно не трогает.
        val llm = FakeLlm("СУТЬ: Просят перезвонить.\nРАСШИФРОВКА:\nПередзвони мені до шостої.")

        LlmSpeechToText(llm).transcribe(recording("audio/ogg"))

        assertTrue("модель не просят о сути по-русски", llm.prompt!!.contains("по-русски"))
        assertTrue("расшифровка перестала быть дословной", llm.prompt!!.contains("дословный текст записи"))
    }
