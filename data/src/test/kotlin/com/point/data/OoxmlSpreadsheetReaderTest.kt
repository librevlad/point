...
/** Pure JVM: read rows back out of an OOXML spreadsheet, whichever way its cells carry text. */
class OoxmlSpreadsheetReaderTest {
    ...
    fun `round-trips the writer's own inline-string xlsx (the failing case)`() = runBlocking {
        // Exactly what «В Excel» produces — the spreadsheet that could not become a PDF.
    ...
    fun `pads gaps from sparse column refs`() = runTest {
        // A cell at column C with no A/B → the row keeps its shape.
