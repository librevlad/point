...
/**
 * Один путь между телефоном и компьютером — ящики сервера (#475, #524).
 *
 * Про общую сеть здесь нет ни слова, и это главное, что тест утверждает: у устройств нет ни
 * адресов друг друга, ни портов, ни возможности «увидеть» друг друга — есть только сервер и два
 * ящика на нём. Прежняя связка требовала совпадения трёх условий разом (одна сеть · открытый экран
 * «Устройства» · нажатие на компьютере за минуту) и вне общей сети не возникала никогда.
 *
 * Сервер поднимается настоящий, но крошечный — ровно три ручки из `relay/point_server/app.py`.
 * Голый сокет, а не `com.sun.net.httpserver`: юнит-тесты Android-модуля идут по своему classpath,
 * и `com.sun.*` там не разрешается (тот же урок, что у `UrlConnectionHttpFilesTest`). Зато
 * по-настоящему проверяются и адресация письма, и заголовок пропуска, и подтверждение забранного.
 */
class PhoneToPcTest {

    // --- маленький сервер Point: ящик на устройство, хозяин берётся из пропуска ---

    private lateinit var server: ServerSocket
    private lateinit var worker: Thread
    private val boxes = mutableMapOf<String, MutableList<Pair<String, ByteArray>>>()

    /** Пропуск → устройство. Чужого устройства в этой таблице нет, как и на сервере. */
    private val passes = mapOf("pass-phone" to "d-phone", "pass-pc" to "d-pc")

    /** Кого сервер знает. Компьютера, которого нет в круге, он не находит — как настоящий. */
    ...
    /** Соединение закрывается каждым ответом: переиспользованный сокет тесту ни к чему. */
    ...
    /**
     * Компьютер разбирает свой ящик и отвечает — то же самое, что делает `RelayPoller` на ПК.
     *
     * Стоит здесь стойкой, а не настоящим классом: `:desktop` живёт в другом модуле, и тащить его
     * сюда значило бы завести зависимость ради теста. Что именно отвечает настоящий ПК, судится у
     * него самого (`RelayRequestsTest`); здесь судится дорога.
     */
    ...
    /** Работа телефона идёт на своём потоке: ответа компьютера ждём по-настоящему. */
    ...
    /** Ждём, пока письмо доедет до ящика: сеть здесь настоящая, а не виртуальное время. */
    ...
    fun `общий буфер едет той же дорогой, и пустой буфер — это ответ`() {
        ...
        waitForLetter("d-pc")
        pcAnswers { _, _, _ -> emptyMap<String, String>() to ByteArray(0) } // у компьютера пусто
    ...
    // --- разные беды различаются на дороге, а не только в словах (#524) ---
    ...
    fun `компьютер в круге, но не запущен — письмо лежит, забирать его некому`() {
        // Никто не отвечает: `pcAnswers` не зовётся вовсе. Прежде это выдавалось за доставку, и
        // человек уходил к выключенному компьютеру за тем, чего там не случилось.
        val outcome = runBlocking { phone(waitSeconds = 1).send(pc, objectOnPhone(), "чек.txt", emptyMap()) }
