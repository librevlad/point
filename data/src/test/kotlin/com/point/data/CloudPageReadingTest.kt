...
/**
 * Второе чтение ложится рядом с первым (#280): свой ключ метаданных, офлайновый слой цел.
 * Без этого сравнивать два чтения было бы не с чем — а вся ценность второго читателя именно в
 * расхождении, которое показывает, где идти перечитывать.
 */
class CloudPageReadingTest {
    ...
    fun `слой пишется в свой ключ и не трогает офлайновое чтение`() = runTest {
        ...
        assertEquals("/scratch/atoms.tsv", merged[META_OCR_ATOMS_REF]) // офлайновое чтение цело
        assertNotNull(merged[META_CLOUD_ATOMS_REF])
        assertTrue(added.keys.none { it == META_OCR_ATOMS_REF }) // затирать нечем — ключа тут нет
    }
