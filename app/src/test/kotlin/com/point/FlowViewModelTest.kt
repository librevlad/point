...
class FlowViewModelTest {
    ...
    ) = FlowViewModel(store, FakeRegistry(caps, cloud, slow, keyNeeding, own) { userKeys.keys().mine.isNotEmpty() }.also { registry = it }, resolver, ChatTalk(chatResponder, store, com.point.core.flow.GraphKnowledge(store, chatPdfText)), enrichment, history, usage, chosenApps, SettingsParts(userKeys, aiFacts, builtInKeys, keyCheck, sensorySettings), consent, appLauncher, pdf, sensory, cloudPrivacy, com.point.core.flow.YoloMode.OFF, snapshot, crashLog, dispatcher, AppIconResolver { null }, PcParts(pcLinks, pcTransport, pcCaps, linkMonitor, PulledFileFactory { name -> java.io.File(java.io.File(System.getProperty("java.io.tmpdir")), "pulled-" + name).absolutePath }), noFrames, com.point.core.flow.PhoneRegion { "UA" }, AccountParts(account, accountClient, circleStore, pendingLogins, deviceKeys), browser, sharedTexts, memory)
