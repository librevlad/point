...
/**
 * Сторож против разговора о сетях (#524, #475): второй дороги между устройствами нет, и слов о ней
 * тоже не должно быть.
 *
 * Отказ «Компьютер недоступен. Проверьте, что порт открыт в брандмауэре Windows (или сделайте
 * Wi-Fi сеть «Частной»)» стоил человеку вечера: он шёл чинить брандмауэр, а причина была другая —
 * компьютер просто не был запущен. Теперь такого совета дать физически нечему: дорога одна, идёт
 * наружу, входящих соединений Point не слушает вовсе.
 *
 * Правило переживёт список нарушений — поэтому здесь не перечень строк, а проверка по исходникам,
 * ровно тем же разбором, что и [NoJargonInProductTest]: комментарии вырезаются, из оставшегося
 * берутся строковые литералы с кириллицей. Значит честное объяснение в комментарии — «слушающий
 * сокет на Windows вызывает окно брандмауэра» — сторожа не будит, а тот же текст в отказе будит.
 *
 * Смотрит и на компьютер тоже (`:desktop`): «Point для ПК» — часть того же продукта, и его экран
 * человек читает теми же глазами.
 *
 * Каждое запретное слово написано выражением, а не корнем: «порт» обязан ловиться, а «портал» —
 * нет, и «сети» обязано ловиться, а «сетка» таблицы — нет. Сторож, падающий на невиновном, учит
 * обходить себя.
 */
class NoNetworkJargonTest {

    /**
     * Слова про устройство сети. Человек не выбирал ни одно из них, повлиять на них не может, а
     * совет их «проверить» уводит от настоящей причины.
     */
    private val networkTalk = listOf(
        ...
        // Именно ТИП сети, а не всякое упоминание связи: «работает без сети» — честная
        // строка про то, что человеку важно, и запрещать её значило бы обеднить продукт
        // ради буквы правила.
    ...
    /** Слова про сопряжение: связывать больше нечего, и звать к несуществующему экрану нельзя. */
    ...
    fun `в продукте не осталось ни одного упоминания сопряжения устройств`() {
        // Связывание кончилось вместе с локальной сетью: устройство попадает в круг входом. Слово
        // «свяжите» звало бы человека к экрану, которого нет.
    ...
    fun `сторож правда прочитал продукт, а не пустоту`() {
        // Сломанный путь до исходников сделал бы проверки выше вечнозелёными — самая дорогая из
        // возможных поломок: она не видна ни в одном прогоне.
    ...
    fun `сторож не путает похожие слова с запретными`() {
        // Портал ждёт объект, сетка таблицы остаётся сеткой, а частота — частотой. Сторож, который
        // роняет сборку на них, будет отключён — и перестанет охранять что-либо вообще.
        val innocent = listOf(
            "Портал ждёт объект",
    ...
    // --- чтение исходников: тот же разбор, что у соседнего сторожа ---
    ...
    /** Строковые литералы файла без комментариев — маленький лексер, как у [NoJargonInProductTest]. */
