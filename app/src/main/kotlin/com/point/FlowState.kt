...
/**
 * С чем человек пришёл на экран ключа и куда он вернётся (#465).
 *
 * До этого экран ключей был местом без «откуда» и без «зачем»: человек тапал «Понять · нужен
 * ключ», ждал минуту, получал отказ, шёл по предложению в настройки — и оказывался перед списком
 * из семи сервисов, где о его объекте не было сказано ни слова. Вернувшись, он видел экран
 * настроек, а не свою фотографию, и заново вспоминал, ради чего всё затевалось.
 *
 * Поручение — это ровно две вещи, которые нельзя терять по дороге: [action] (какое действие
 * послало) и [objectName] (к чему возвращаться). Ими экран говорит, зачем ключ ИМЕННО здесь, и
 * ими же называет дверь обратно.
 *
 * Чего поручение НЕ делает — так это не запоминает действие, чтобы выполнить его самому. Это
 * прямо против инварианта «Point никогда не строит автоматические цепочки»: человек вернётся к
 * объекту, где действие стоит доступным и ждёт его тапа. Поэтому здесь имя, а не `CapabilityId`:
 * запускать по имени нечего, и соблазна не возникнет.
 */
data class KeyErrand(
    /** Имя действия без приписки про ключ: «Понять», «Перевести», «Расшифровать». */
    val action: String,
    /** Как называется объект, к которому человек вернётся: «чек.jpg» или «Изображение». */
...
data class FlowUiState(
    ...
    /** Поручение, с которым сюда пришли (#465): какое действие послало и куда возвращаться.
     *  null — пришли сами, дверью «AI-ключ»: возвращать некуда, объекта на экране нет. */
