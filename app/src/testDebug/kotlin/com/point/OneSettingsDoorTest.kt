...
class OneSettingsDoorTest {
    ...
    @Test fun `экран ничего не обещает — он просто показывает, что за ним лежит`() {
        ...
        // Прежде экран открывался абзацем: перечислением того, что видно строкой ниже, и фразой
        // «Больше Point ни о чём не спрашивает». Перечисление дублировало раскладку, а фраза была
        // похвальбой — обещанием, которое приходилось сторожить тестом (#580). Теперь обещания
        // нет вовсе, и сторожить нечего: правду говорят сами двери, проверенные выше.
