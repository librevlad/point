...
def test_имя_письма_говорит_когда_его_положили(root):
    """Возраст письма — часть уговора, а не случайность имени (#1321).

    По нему получатель понимает, ждёт ли ещё ответа тот, кто просил: выключенный компьютер
    забирает просьбу часами позже, чем телефон перестал слушать, и отвечать уже некому —
    исход поедет очередью. Собранное иначе имя убрало бы эту развилку молча.
    """
    before = time.time()
    bid = mailbox.push(root, "u1", "dev1", "письмо".encode("utf-8"))
    after = time.time()

    put = int(bid.split("-")[0]) / 1_000_000_000
    assert before <= put <= after
