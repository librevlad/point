...
def create_app(
    ...
    def knock(
        ...
        `knocked` — про почту, а не про телефон (#1108). Сервер знает ровно одно: взяла ли
        почта письмо. Проснулся ли от него закрытый Point, отсюда не видно — Google
        принимает письмо и для выключенного телефона, а остановленное человеком
        приложение Android не будит вовсе. Поэтому «разбудили» здесь не обещается:
        проснулся телефон или нет, видит компьютер — по тому, пришли к нему за просьбой
        или так и не пришли.
        """
