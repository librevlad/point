...
class Knocker:
    ...
    def knock(self, token: str) -> bool:
        """Постучать в одно устройство. `False` — устройство больше не отвечает."""
        if not token:
            raise Silent("у устройства нет адреса для стука")

        # Письмо без заголовка и текста: показывать его будет телефон сам, когда сходит
        # за просьбой. Заголовок здесь означал бы, что содержимое едет через Google.
        message = {
            "message": {
                "token": token,
                "data": {"knock": "outbox"},
